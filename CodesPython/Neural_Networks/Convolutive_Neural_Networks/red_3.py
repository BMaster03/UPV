...
def managing_images():
    ...
    """
    current_path = Path(__file__).parent # me dice donde está este archivo
    images_folder = current_path.parent / "images" # me permite extraer la ruta del folder completo 
    image_path = images_folder / 'artificial_vision.jpg'
    img = cv2.imread(image_path) # lee la imagen 
    print(img)
    print(img.shape) # es un tensor, me dice que tamaño tiene la imagen 

    plt.imshow(img)
    plt.axis('off')
    plt.show() # muestra la imagen en una figura con matplotlib
    ...
    aux_dim = np.zeros((img_rgb.shape[0], img_rgb.shape[1])) # matriz de puros ceros, para generar un tensor 

    red_color = np.dstack((red, aux_dim, aux_dim)).astype(np.uint8) # me rellena las capas que no estoy llamad
    green_color = np.dstack((aux_dim, green, aux_dim)).astype(np.uint8)
    blue_color = np.dstack((aux_dim, aux_dim, blue)).astype(np.uint8)

    axs_orgl = plt.subplots(1, 3, figsize=(16, 8))[1] # lo regresa por la tupla que se genera antes el axs_orgl 
